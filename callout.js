  ...
  Callout.prototype = {
    ...
    _onRender : function () {
      ...
      $currentStepBtn = $('<div class="introjs-current-step">' +
        ...
        parseInt(self._currentStep + 1, 10) + ' of ' + parseInt(self._introItems.length, 10) +
      ...
      $currentStepBtn.on('click', '.introjs-button', function (e) {
        e.stopPropagation();
        e.preventDefault();
        instance._toggleGoToStepMenu(), 10;
      });
    ...
    _renderSubMenu : function () {
      ...
      this._introJs._introItems.forEach(function (item, index) {
        ...
        $subMenu.find('tbody').append('' +
          '<tr index=' + index + '>' +
          '<td>' + this._padZero(parseInt(index + 1, 10)) + '</td>' +
      ...
      $subMenu.off('click').on('click', 'tr', function (e) {
        e.stopPropagation();
        e.preventDefault();
        this._goToStepMenu.call(this, parseInt($(e.target).parents('tr').attr('index'), 10));
      }.bind(this));
